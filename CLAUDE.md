...
rate a film from the members whose ratings track his. The scraping stays
local; the scoring runs in `run()` each day (`for_you.py`, database
aggregates plus a film page and a JustWatch lookup per new pick) and feeds
the dashboard's **For you** tab and the taste sections on every film page.
It hasn't yet been shown to beat simply trusting the Letterboxd average
(`--taste-eval`), so the page presents its numbers as estimates, beside
that average.
...
| `taste.py` | The taste engine (user-user collaborative filtering). Collects: `screening_plan` screens the fans of Josh's four favourites (`/film/<slug>/fans/`, from `config/taste.yaml`; stored under the sentinel score `FANS_STARS` = 0 and worth `FAVOURITE_HITS` = 3 hits an appearance), who else gave his 5★ films 5★ (`--five-star-pages` deep), and `/film/<slug>/members/rated/<stars>/` for his most distinctive ratings (furthest from the Letterboxd average), plus who he follows, then scrapes the most promising candidates' `/films/` grids through `PoliteFetcher` (randomised delays, a request cap, stop on the first block). Scores: Pearson over shared films of residuals (rating minus corpus mean, the rater's own offset and the film's), shrunk by overlap, top-N neighbours' weighted residuals damped toward the baseline. `evaluate` holds out Josh's ratings and compares against "Letterboxd average + your offset". Screened films (his most distinctive ratings) are held out too, but without the raters they recruited — found *because* they gave his exact rating there: the primary numbers leave out only recruits who wouldn't have been scraped without that film (`Recruitment.kept_without` replays the scrape queue, `screen_hits desc, id`, against each run's last-taken rater), repeated leaving out every recruit and none, as bounds. Since the screened films were picked for being far from the Letterboxd average, the headline is all films pooled, with paired bootstrap intervals (clustered by director) on a fixed list of `KEY_METHODS`. It also scores the simple swap with Josh's four favourites counted several times over in the neighbour correlation (`FAVOURITE_WEIGHTS`; exploratory — on 2026-09-26 no weight beat the unweighted simple swap). A placebo, "Letterboxd avg recalibrated to you" (one slope, fitted on the training ratings), shows what rescaling to Josh's ratings gets for free: a gain in stars of error only counts as taste if "Placebo + twins" beats it; it can't reorder films, so for ranking (Spearman) the plain baseline is the comparison. A screened film whose recruits aren't recorded yet is held back, and the time split waits until none are. A rater's pages count as known only from `screening`/`first-found` rows (`CERTAIN_SOURCES`) — screening pages turn over within hours, so a re-fetch proves nothing — and where those don't account for every screening hit, every screened film the rater gave that score to after being found counts as one they may have been recruited through. It also scores a Letterboxd-centred variant (`TasteParams.baseline="letterboxd"`): every residual measured from the film's Letterboxd average instead of the corpus's own shrunk estimate, which from a few dozen raters leaves much of each film's quality in the residual — so the taste layer can sit on top of the Letterboxd average without counting that quality twice (the "simple swap" row shows what happens if it does). Only films with a stored Letterboxd average (diary `rating`) get the re-centring; `recommend` doesn't use it yet. `recommend` leaves TV out of its picks: `films_only` walks the best-predicted films in order, asking `FilmKindLookup` (the cached `tmdb_kind`, else the film page's TMDB link, fetched through a `PoliteFetcher`) only until it has enough. The SQL itself lives in `db.py` |
| `similar.py` | TMDB-correlated discovery (`because_you_watched`, by director/cast/genre, hidden gems, popular, rewatch); `with_availability` is its JustWatch step on its own, for candidates that already have Letterboxd details (For you's picks) |
| `for_you.py` | The For you tab's daily data — the taste engine run for the dashboard in `run()`: an estimate for each watchlist film enough neighbours rated, 24 new picks (best estimate first, TV skipped by the film page's TMDB link, enriched like a discovery section and stored in `discovery_films`), "because you loved" per estimated film (Josh's 4½★+ films its fans among his neighbours love unusually often: share of its fans minus share of all neighbours, so a crowd-pleaser explains nothing), and "if you like this, see…" per film the page ships (the same lift across the whole corpus). Other members' usernames never leave it except Sarah's — the page is public. Stored as `meta.for_you`; a failure carries yesterday's forward, picks and all |
| `cinemas.py` | Scrapes showtimes for 4 London cinemas (Prince Charles, Barbican, Vue Fulham Broadway, Riverside Studios) — one fetcher per venue, each a different mechanism (plain HTML, a JSON API, an opaque-token AJAX endpoint). Vue's own API is behind a Cloudflare challenge from datacenter IPs, so in Actions its listing comes from CinemaGuide's backend instead (see Known constraints). `drop_past_showings` is what makes a showing fall off once it's started — applied when `run()` stores the listing and again at dashboard build, both in London time. `clean_listing_title` strips what the venue added and the film doesn't have ("(10th Anniversary)", "- IMAX", a re-release year); `match_watchlist_film` matches a listing against the watchlist by title, and `resolve_listing_to_letterboxd` finds the Letterboxd film for everything else (TMDB for the id, then `/tmdb/<id>/` for the slug), with its two network calls injected so the matching judgement is testable without either |
| `dashboard.py` | Builds the dashboard's JSON payload from `StateDoc` and renders `dashboard.html` (template + embedded JS live in this one file); `_search_taxonomy` is what lets the page classify a *searched* film's offers without duplicating `brands.py`/`config.py` in JS. The film detail view (the long look behind quick look's "Full details") adds no payload of its own — its director/cast/genre relations are derived in JS from `films_by_slug`, which already carries every watchlist and discovery film — TMDB's own answer to the same questions arrives separately, from the Worker, and is merged in behind the local one. The person page (any director/actor name is a link to it) is the same shape one level up, and shares the film page's back-trail: `detailTrail` holds both kinds of stop, so film → director → another of their films unwinds one step at a time. The bottom nav's five destinations are Home / Lists / Services / Films / Cinemas: Home is the time-sensitive rows (what just landed on a service you have, what's leaving) plus discovery, ending on "Got 90 minutes?"; Lists holds `config/custom_lists.yaml` (its own `list_sections` payload key — fourteen of them inline pushed everything actionable off Home). A "By VPN country" tab used to sit where Lists does; it was removed because the Films tab already does the same job with strictly more per-film detail, and its payload was `countries` at 6MB of a 13MB page. What survives it is the `countries` index (code/name/film_count) that names country badges, and `HOME_COUNTRY_CODES`. **For you** (`view-foryou`, `renderForYou`) took Lists' slot in the phone bottom nav (five buttons is all it holds; Lists is a button on Home there, `listsBtnMobile`, and keeps its desktop tab): a hero for the best-estimate unseen watchlist film on a service you have, the watchlist ranked by estimate, the picks as a poster grid, and an anonymised taste-matches panel. `_for_you_data` narrows `meta.for_you` to films the page actually ships, dropping dismissed recommendations; `forYouFilmSections` adds "What your taste matches think" and "If you like this, see…" to every film page, ahead of the live TMDB layer |
...
| `custom_lists.py` | User-defined watchlist subsets from `config/custom_lists.yaml` (rules on director/starring/year, Letterboxd lists as sources, manual include/exclude) — each becomes a section on the Lists tab, recomputed against the current watchlist at build time |
...
| `meta` | `run()`, full replace | `last_run_at`, `last_justwatch_check_date`, `last_seen_diary_guid`, `recent_watches`, `recent_additions`, `for_you` (see `for_you.py`; ~100KB, null until there's enough to go on) |
...
tune with `--screen-films`, `--five-star-pages`, `--max-raters`, `--request-delay`,
...
- **Vue blocks datacenter IPs.** myvue.com (page and API alike) answers
  ...
  directly first (it works from a home connection), then falls back to
  CinemaGuide's public Firebase endpoint, which carries Vue's full
  forward programme with Vue's own booking links. Unofficial: if it
  changes shape the fetch fails soft like any other venue.
- **Past showings fall off in three places**: `run()` drops them before
  storing (so a venue that keeps failing can't carry a stale listing
  forward forever), `dashboard.py` drops them at build, and the page
  drops them again on load, since it can be open long after its build.
