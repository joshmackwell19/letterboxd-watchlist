...
def bucket_offers(
    offers, favorites: set[tuple[str, str]], revisitable: set[str]
) -> dict[str, list[tuple[str, str]]]:
    """Classify every (brand, country) an offer list covers into one of four
    buckets, most-actionable first: already favourited, could get again
    (friends/family/resubscribe), free/ad-supported, or needs a subscription
    you don't have. Shared by the text and HTML report renderers so the
    categorization logic lives in exactly one place.
    """
